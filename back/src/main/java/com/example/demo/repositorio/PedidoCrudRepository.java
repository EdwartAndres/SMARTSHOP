...
public interface PedidoCrudRepository extends CrudRepository<pedidos,Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE pedidos SET can_Total=:can_Total,costoNeto=:costoNeto  WHERE cod_Pedido=:cod_Pedido", nativeQuery = true)
    void actualizarPedidos(@Param("can_Total") int can_Total, @Param("costoNeto")int costoNeto,@Param("cod_Pedido") Integer cod_Pedido);
